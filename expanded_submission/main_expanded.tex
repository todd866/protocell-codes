...
\medskip
\noindent\textit{Note:} This protocol uses well-characterized components (defined lipids, single oscillator, controlled coupling) for experimental tractability. Real prebiotic conditions would be \textit{messier}, not simpler---hundreds of organic species, multiple competing reaction networks, heterogeneous membrane compositions. Our framework predicts that code emergence should be \textit{more} robust under such high-dimensional conditions, not less, because substrate competition and coordination pressure are generic features that do not require chemical specificity.
