...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/fig_cycles_sweep.pdf}
\caption{\textbf{Temporal cycles expand code capacity.} (A) Unique codes increase from 12 (1 cycle) to 32 (4+ cycles). Single-cycle codes cannot distinguish all 32 environments. (B) Decoding accuracy improves from 71\% to 98\% as temporal structure enables richer discrimination. (C) Mutual information approaches the theoretical maximum of 5 bits (for 32 classes) with 4 cycles. \textbf{Implication:} Temporal sequence structure is not optional---it is required for full expressivity.}
\label{fig:cycles}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/fig_kappa_sweep.pdf}
\caption{\textbf{Coupling strength $\kappa$ controls different aspects of code emergence.} (A) Discretization (bimodality) is relatively stable across $\kappa$---competition drives digitization regardless of coupling. (B) Interface separation peaks at intermediate coupling ($\kappa \approx 0.3$): too little coupling yields idiosyncratic codes, too much yields over-synchronization. (C) Cross-receiver decoding accuracy increases with coupling as shared conventions emerge, then declines when over-coupling collapses distinctiveness. \textbf{Interpretation:} Competition digitizes; coupling standardizes.}
\label{fig:kappa}
\end{figure}
...
\caption{\textbf{Confusion matrices for chemistry-based code emergence.} (A) Standard chemistry (15 species) shows clustering: multiple environments map to the same attractor, producing off-diagonal errors. Only 12 distinct clusters emerge from 32 environments. (B) Messy chemistry (50 species) achieves perfect diagonal structure: each environment maps to a unique attractor. The additional degrees of freedom expand the attractor landscape, enabling full discrimination.}
\label{fig:confusion}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{figures/fig_raid_robustness.pdf}
\caption{\textbf{Predator-prey dynamics are robust to raid function form.} We tested three alternative raid success functions: (A) log-ratio (original), (B) power-law, and (C) threshold-based. All three produce bimodal size distributions with predators clustering near the coherence ceiling and prey remaining large. The qualitative structure---ecological differentiation without specialized machinery---is not an artifact of a particular functional form.}
\label{fig:raid}
\end{figure}
