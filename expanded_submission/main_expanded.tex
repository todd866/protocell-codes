...
\begin{tabular}{lrrr}
\toprule
\textbf{Condition} & \textbf{Unique Codes} & \textbf{Reproducibility} & \textbf{Sep$_{\text{attractor}}$} \\
\midrule
...
\begin{equation}
\tau_{\text{prop}}(N) = \frac{L^2}{D} \approx \frac{N}{\rho \, D}
\end{equation}
where $D$ is the diffusion coefficient, $\rho$ is packing density, and $L = \sqrt{N/\rho}$ is the characteristic diameter. Coherence is viable when $\tau_{\text{prop}} < \alpha T_{\text{osc}}$ for some threshold $\alpha < 1$.

This yields a \textbf{derived ceiling}:
\begin{equation}
N^* = \alpha \, \rho \, D \, T_{\text{osc}}
\end{equation}

For plausible prebiotic parameters ($D \sim 10$ $\mu$m$^2$/s for small signaling molecules, $T_{\text{osc}} \sim 100$ s chemical oscillation, $\rho \sim 0.1$ cells/$\mu$m$^2$, $\alpha = 0.5$), this yields $N^* \approx 50$, consistent with our simulation range.
