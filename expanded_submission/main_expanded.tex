...
The substrate competition mechanism (Section 3) demonstrates \textit{how} codes can emerge. But a skeptic might ask: is this mechanism-specific, or is code emergence a generic outcome of coordination pressure? To address this, we implemented an independent validation using Lewis signaling games \cite{lewis1969convention,skyrms2010signals}.
...
Code emergence is \textbf{mechanism-independent}. Any system with:
\begin{itemize}
    \item High-dimensional internal dynamics
    \item Coordination pressure between units
    \item Some form of winner-take-most or discretization
\end{itemize}
will spontaneously develop symbolic codes. This is not a property of substrate competition specifically; it is a generic consequence of coordination under observability constraints.
...
Khrennikov and Yurova Axelsson \cite{khrennikov2024} recently showed that all 24 known genetic codes can be derived from a universal dynamical function using 2-adic (p-adic with $p = 2$) number theory. Their encoding places nucleotides in a 2-bit representation, making wobble-position changes (position 3) correspond to minimal p-adic distance.
...
This validates Khrennikov's formalism: the 2-adic metric is not arbitrary---it captures the biologically meaningful structure of the genetic code. Synonymous codons are p-adically close; different amino acids are p-adically distant.
...
For plausible prebiotic parameters ($v \sim 10$ $\mu$m/s diffusive signaling, $T_{\text{osc}} \sim 100$ s chemical oscillation, $\rho \sim 0.1$ cells/$\mu$m$^2$, $\alpha = 0.5$), this yields $N^* \approx 25$--$60$, consistent with our simulation range.
...
\textbf{Lewis signaling games} (Section 5): An independent validation where sender-receiver pairs evolve shared codes through coordination pressure alone, without any substrate competition physics.

Both mechanisms produce the same outcome: discrete, non-colliding code mappings with high consistency. This demonstrates that code emergence is \textbf{mechanism-independent}---a generic consequence of coordination pressure under observability constraints, not a quirk of any particular physical implementation.
...
\textbf{Phenomenological dynamics.} The reservoir model is a generic proxy for high-dimensional reaction networks, not literal chemistry. Future work should implement explicit chemical kinetics, potentially building on established protocell simulation platforms \cite{shirtediss2025}.
...
We have shown that symbolic codes emerge generically from coordination pressure in coupled compartment networks. Two independent mechanisms---substrate competition and Lewis signaling games---produce equivalent results, demonstrating that code emergence is mechanism-independent.
