...
\end{quote}
We invert this sequence: \textit{social dynamics preceded molecular codes}:
\begin{quote}
...
\textit{Code signal}: The 60-dimensional vector $\mathbf{c} = [\boldsymbol{\phi}_{\text{center}}; \boldsymbol{\phi}_{\text{edge}}]$ comprising competitive outputs (Eq.~\ref{eq:substrate}) from center and edge vesicles, accumulated over 4 temporal cycles.\\[0.3em]
...
Modern microfluidic techniques allow fabrication of vesicle arrays with controlled topology, loading with oscillatory chemistry, and measurement of inter-vesicle coupling. The predictions---that coupled vesicle networks spontaneously develop reproducible discrete codes---are directly testable.
...
I thank Cyril Grueter (University of Western Australia; now University of Oxford) for undergraduate teaching in biological anthropology that first made me see social coordination as a problem physics could address.
