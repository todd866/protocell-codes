...
PARAMS = {
    ...
    # SMOOTH CEILING: Coherence limits size through soft constraint
    # Physics: coherent oscillation propagation time ~ sqrt(N), must stay below period
    # High-C colonies face increasingly harsh size penalties, not a hard cutoff.
    'N_ceiling': 40,        # Characteristic scale for high-C colonies
    'ceiling_sharpness': 8.0,  # Controls steepness of soft ceiling (higher = sharper)
...
def smooth_ceiling(N, C, params):
    """Apply smooth size ceiling based on coherence.

    Physics motivation: coherent oscillation requires synchronization across
    the colony. Propagation time scales as sqrt(N). At high C, the system
    demands tight phase relationships that become impossible above a critical
    size. Rather than a hard cutoff, we model this as increasingly steep
    fitness penalty for exceeding the coherence-dependent ceiling.

    The ceiling function:
        N_eff(C) = N_ceiling * (1 - C^2) + N_min * C^2

    At C=0: ceiling is effectively infinite (low-C can be any size)
    At C=1: ceiling drops to N_ceiling (high-C limited)

    The soft constraint applies a restoring force proportional to how far
    above the ceiling the colony has grown.
    """
    N_ceil = params['N_ceiling']
    N_min = params['N_min']
    sharpness = params['ceiling_sharpness']

    # Coherence-dependent ceiling: higher C = lower ceiling
    # Quadratic in C so low-C colonies are nearly unconstrained
    C2 = C ** 2
    effective_ceiling = N_ceil + (params['N_max'] - N_ceil) * (1 - C2)

    # Soft constraint: pull N toward ceiling if exceeded
    # Uses smooth sigmoid-like decay above ceiling
    overshoot = N - effective_ceiling

    # Apply soft constraint only where overshooting
    # penalty = sharpness * max(0, overshoot) implements gradual pullback
    N_constrained = np.where(
        overshoot > 0,
        effective_ceiling + overshoot / (1 + sharpness * overshoot / effective_ceiling),
        N
    )

    # Floor at N_min
    N_constrained = np.maximum(N_constrained, N_min)
...
def cost(N, C, params):
    ...
    Note: Size differentiation comes from SMOOTH CEILING in selection_step(),
    combined with this cost function. The ceiling implements the physical
    constraint (propagation time ~ sqrt(N) must stay below oscillation period),
    while this cost shapes the fitness landscape.
    """
...
def selection_step(N, C, R, params, rng):
    ...
    # ==========================================================================
    # SMOOTH CEILING: High-coherence colonies face continuous size constraint
    # Physics: coherent oscillation propagation time ~ sqrt(N), must stay < period
    # Rather than a hard cutoff, we apply a soft constraint that makes large
    # high-C colonies increasingly difficult to sustain.
    # ==========================================================================
    N = smooth_ceiling(N, C, params)
